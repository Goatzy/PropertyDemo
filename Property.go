...
/*此函数用于提醒用户输入抽样百分占比，返回这个百分占比*/
func GetProportion() int {
	fmt.Println("Please input proportion of this selective examination in percentage(1~100): \n ")
    var proportion int
    fmt.Scanf("%d",&proportion)
	return proportion
}
...
func ShortestPathDisplay(matrix [][MAX]int,chosen []int){
	bestPath:=INF
	num:=len(chosen)
	...
	fmt.Println("The length of the shortest path is:",bestPath)
	fmt.Println("The shortest path is:")
	for i:=0;i<num ;i++  {
		fmt.Printf("%d——>",chosen[bestPathX[i]])
	}
	fmt.Println(source)
}
...
func main() {
	//SaveMongoDB()
	fmt.Println("****Bank Fixed Assets Random Inspection's Best Path Planning****")
			prop:=GetProportion()
			chosen:=GetRandom(prop)
			mat:= GetMatrix(chosen)
			ShortestPathDisplay(mat,chosen)
}
